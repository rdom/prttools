...
Double_t tdcRefTime[prt_ntdcm], gTotO[prt_maxchm], gTotP[prt_maxdircch][10],
  gLeOffArr[prt_maxdircch], gEvtOffset(0);
TGraph *gGrIn[prt_maxchm], *gLeO[prt_maxchm], *gGr[maxfiles][prt_maxchm], *gGrDiff[prt_maxchm];
TCanvas *cTime;

const int nch_tdc = 32; // 48
...
Double_t getTotWalk(Double_t tot, Int_t ch, Int_t type = 0) {
  ...
  if (type == 0) {
    ...
    Double_t wcorr(8);
    if (ch / nch_tdc == 1) wcorr = 0;
    if (ch / nch_tdc == 5) wcorr = 0;
    if (ch / nch_tdc == 9) wcorr = 0;
    if (ch / nch_tdc == 13) wcorr = 0;
...
void TTSelector::SlaveBegin(TTree *) {
  ...
  for (Int_t j = 0; j < nfiles; j++) {
    for (Int_t c = 0; c < prt_maxch; c++) {
      hFine[j][c] = new TH1F(Form("hFine_%d_ch%d", j, c),
                             Form("hFine_%d_ch%d;bin [#];LE entries [#]", j, c), 600, 0, 600);
      hTot[j][c] =
        new TH1F(Form("hTot_%d_ch%d", j, c), Form("hTot_%d_ch%d;TOT [ns];entries [#]", j, c), totb,
                 totl, toth); // 2000  50 80 //400 -2 2
  ...
  for (Int_t c = 0; c < prt_maxch; c++) {
    hLeTot[c] = new TH2F(Form("hLeTot_ch%d", c), Form("hLeTot_ch%d", c), 1000, le1, le2, 100, 0,
                         12); // 35 40 for 1345
  ...
  for (Int_t m = 0; m < prt_nmcp; m++) {
    for (Int_t p = 0; p < prt_npix; p++) {
      ...
      hTimeL[m][p] = new TH1F(Form("hTimeL_mcp%dpix%d", m, p),
                              Form("hTimeL_%d_%d;LE time [ns];entries [#]", m, p), leb, le1, le2);
      hTimeT[m][p] = new TH1F(Form("hTimeT_mcp%dpix%d", m, p),
                              Form("hTimeT_%d_%d;TOT [ns];entries [#]", m, p), 400, le1, le2);
      hShape[m][p] = new TH2F(Form("hShape_mcp%dpix%d", m, p),
                              Form("hShape_%d_%d;time [ns];offset to the threshold [mV]", m, p),
                              400, le1, le2, 130, -5, 35);
  ...
  for (Int_t i = 0; i < prt_ntdc; i++) {
    hSigma[i] =
      new TH1F(Form("hSigma%d", i), "ch-ref. resolution;sigma [ns];entries [#]", 200, 0, 0.1);
...
Bool_t TTSelector::Process(Long64_t entry) {
  ...
  Double_t triggerLe(0), triggerTot(0), grTime0(0), grTime1(0), grTime2(0), timeLe(0), timeTe(0),
    offset(0);
  ...
  for (int i = 0; i < Hits_ && i < maxhits; i++) {
    ...
    if (gcFile != "0") {
      ...
      Double_t xx, yy;
      gGrIn[prt_addRefChannels(ch + 1, tdc)]->GetPoint(Hits_nFineTime[i], xx, yy);
      timeL[i] -= yy; // fast
        ...
        if (ch < prt_maxch) {
          ...
          if (mcp < prt_nmcp) {
            ...
            hFine[fileid][ch]->SetTitle(
              Form("tdc 0x%s, chain %d, lch %d = ch %d, m%dp%d, col %d, row %d ", tdchex.Data(),
                   (ch / 16) % 2, ch % 16, ch, mcp, pix, map_col[ch], map_row[ch]));
...
TString drawHist(Int_t m, Int_t p) {
  ...
  if (gComboId == 0) {
    ...
    leg->SetBorderSize(0);
    Int_t num = 0;
    ch = prt_addRefChannels(ch, ch / nch_tdc) + 1;
    for (Int_t j = 0; j < nfiles; j++) {
      if (gGr[j][ch]->GetN() < 1) {
        gPad->Clear();
...
TGraph *getGarph(TH1F *hist) {
  ...
  for (Int_t b = 0; b < nbins; b++) {
    ...
    integral -=
      hist->GetBinContent(bmin) * (xmin - axis->GetBinLowEdge(bmin)) / axis->GetBinWidth(bmin);
    integral -=
      hist->GetBinContent(bmax) * (axis->GetBinUpEdge(bmax) - xmax) / axis->GetBinWidth(bmax);
    if (integral >= 0) {
...
void Calibrate() {
  ...
  if (gMode == 5)
    for (Int_t i = 0; i < prt_nmcp; i++) prt_hdigi[i]->Reset();
  ...
  for (Int_t j = 0; j < nfiles; j++) {
    for (Int_t c = 0; c < prt_maxch; c++) {
      ...
      if (gMode == 5) {
        ...
        Int_t ch = c - 1;
        if (ch % (nch_tdc + 1) == 0) continue;
        ch = nch_tdc * (ch / (nch_tdc + 1)) + ch % (nch_tdc + 1);
        Int_t m = ch / 64;
  ...
  for (Int_t m = 0; m < prt_nmcp; m++) {
    for (Int_t p = 0; p < prt_npix; p++) {
      if (hTimeL[m][p]->Integral() > 100) {
        ...
        hRefDiff->Fill(sigma);
        Int_t ch = map_mpc[m][p];
        hSigma[(ch / (nch_tdc)) % 4]->Fill(sigma);
      }
    }
  }
...
void TTSelector::Terminate() {
  ...
  for (Int_t m = 0; m < prt_nmcp; m++) {
    for (Int_t p = 0; p < prt_npix; p++) {
      hTimeL[m][p] =
        dynamic_cast<TH1F *>(TProof::GetOutput(Form("hTimeL_mcp%dpix%d", m, p), fOutput));
      hTimeT[m][p] =
        dynamic_cast<TH1F *>(TProof::GetOutput(Form("hTimeT_mcp%dpix%d", m, p), fOutput));
      hShape[m][p] =
  ...
  for (Int_t c = 0; c < prt_ntdc; c++)
    hSigma[c] = dynamic_cast<TH1F *>(TProof::GetOutput(Form("hSigma%d", c), fOutput));
...
void tdisplay(TString inFile = "file.hld.root", Int_t trigger = 0, Int_t mode = 0,
              Int_t workers = 4, TString cFile = "calib.root", Int_t entries = 0,
              Int_t setupid = 2018) {
