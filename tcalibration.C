...
Double_t tofpi2[]={0,0,72.50,72.50,72.40, 72.20,72.20,72.20,72.10};

Double_t tofp1[] ={0,0,80.80,75.80,73.80, 73.20,72.65,72.40,72.30};
...
Bool_t TTSelector::Process(Long64_t entry){
  ...
  if(gMode==5){
    if(mult1!=1 || mult3!=1 || mult4<1 || mult2<1){ //  || mult2!=1 || mult5!=1
      fEvent->Clear();
      delete fEvent;
      return kTRUE;
    }
