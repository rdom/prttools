...
Int_t mult[prt_maxchm] = {0}, gComboId(0), gTrigger(0), gSetup(2019), gMode(0), gWorkers(4),
      gEntries(0); // 3for2015
Double_t gTimeCutMin(-10000), gTimeCutMax(10000), gTofMin(0), gTofMax(0);
Double_t gMultCutMin(0),
  gMultCutMax(0), gTimeCuts[prt_nmcp][prt_npix][2], gTotMean[prt_nmcp][prt_npix];
...
TH1F *hPTime[prt_nmcp][prt_npix], *hPiTime[prt_nmcp][prt_npix], *hSTime[prt_nmcp][prt_npix],
  *hPTot[prt_nmcp][prt_npix], *hPMult[prt_nmcp][prt_npix], *hEMult[prt_nmcp + 1];
...
void MSelector::SlaveBegin(TTree *) {
  ...
  source >> gSetup >> gMode >> gTrigger >> bins1 >> min1 >> max1 >> bins2 >> min2 >> max2 >>
    gTimeCutMin >> gTimeCutMax >> gMultCutMin >> gMultCutMax >> gsTimeCuts >> gsTotMean >>
    gTofMin >> gTofMax;
  ...
  for (Int_t m = 0; m < prt_nmcp; m++) {
    ...
    for (Int_t p = 0; p < prt_npix; p++) {
      hPTime[m][p] =
        new TH1F(Form("le_mcp%dpix%d", m, p), Form("mcp %d, pixel %d", m, p), bins1, min1, max1);
      hPiTime[m][p] =
        new TH1F(Form("lepi_mcp%dpix%d", m, p), Form("mcp %d, pixel %d", m, p), bins1, min1, max1);
      hSTime[m][p] =
        new TH1F(Form("les_mcp%dpix%d", m, p), Form("mcp %d, pixel %d", m, p), bins1, min1, max1);
      hPTot[m][p] =
        new TH1F(Form("tot_mcp%dpix%d", m, p), Form("mcp %d, pixel %d", m, p), bins2, min2, max2);
      hPMult[m][p] =
        new TH1F(Form("mult_mcp%dpix%d", m, p), Form("mcp %d, pixel %d", m, p), 50, 0, 50);

      if (gMode == 1) {
        hShape[m][p] =
          new TH2F(Form("hShape_mcp%dpix%d", m, p), Form("hShape_%d_%d;LE [ns];offset [mV]", m, p),
                   bins1, min1, max1, 100, -5.1, 15);
        hLeTot[m][p] =
          new TH2F(Form("hLeTot_mcp%dpix%d", m, p), Form("mcp %d, pixel %d;LE [ns];TOT [ns]", m, p),
...
Bool_t MSelector::Process(Long64_t entry) {
  ...
  for (auto hit : prt_event->getHits()) {
    ...
    if (gMultCutMin != gMultCutMax && (thitCount2 < gMultCutMin || thitCount2 > gMultCutMax))
      continue;
    ...
    if (gsTimeCuts != "0" &&
        (timeDiff < gTimeCuts[mcp][pix][0] || timeDiff > gTimeCuts[mcp][pix][1]))
      continue;
    else if (gTimeCutMin != gTimeCutMax && (timeDiff < gTimeCutMin || timeDiff > gTimeCutMax))
      continue;
    hitCount2++;
  ...
  if (gComboId == 0 || gComboId == 2 || gComboId == 5 || gComboId == 4 || gComboId == 10 ||
    ...
    if (selected->InheritsFrom(TH2::Class())) {
      ...
      if (gComboId == 7) {
        ...
        int nct = (prt_geometry == 2023) ? 32 : 48;
        ps->AddText(Form("PADIWA ch =  %d", (ch % nct) % 16 + 1));
      }
      cTime->Update();
...
void MSelector::Terminate() {
  for (Int_t m = 0; m < prt_nmcp; m++) {
    ...
    for (Int_t p = 0; p < prt_npix; p++) {
      ...
      hPiTime[m][p] =
        dynamic_cast<TH1F *>(TProof::GetOutput(Form("lepi_mcp%dpix%d", m, p), fOutput));
      ...
      hPMult[m][p] =
        dynamic_cast<TH1F *>(TProof::GetOutput(Form("mult_mcp%dpix%d", m, p), fOutput));
      if (gMode == 1) {
        hLeTot[m][p] =
          dynamic_cast<TH2F *>(TProof::GetOutput(Form("hLeTot_mcp%dpix%d", m, p), fOutput));
        hShape[m][p] =
          dynamic_cast<TH2F *>(TProof::GetOutput(Form("hShape_mcp%dpix%d", m, p), fOutput));
      }
    }
  }
  for (Int_t i = 0; i < maxMult; i++) {
...
void MyMainFrame::DoDraw() {
  ...
  TString option = Form("%d %d %d %s %s %s %s %s %s %s", gSetup, gMode, gTrigger, fEdit1->GetText(),
                        fEdit2->GetText(), fEdit3->GetText(), fEdit4->GetText(), gsTimeCuts.Data(),
                        gsTotMean.Data(), fEdit5->GetText());
...
void MyMainFrame::DoSlider(Int_t pos) {
  if (fCheckBtn1->GetState() != kButtonDown) prt_drawDigi(prt_geometry, pos, 0, cDigi);
...
MyMainFrame::MyMainFrame(const TGWindow *p, UInt_t w, UInt_t h) : TGMainFrame(p, w, h) {
  ...
  fCheckBtn2 =
    new TGCheckButton(fHm1, new TGHotString("3 sigma time cut                         "), -1);
  ...
  fCheckBtn3 =
    new TGCheckButton(fHm2, new TGHotString("Walk correction                         "), -1);
  ...
  fHm2->AddFrame(new TGLabel(fHm2, "Ch of interest: "),
                 new TGLayoutHints(kLHintsBottom | kLHintsLeft, 5, 5, 3, 4));
  fNumber2 =
    new TGNumberEntry(fHm2, 0, 9, 999, TGNumberFormat::kNESInteger, TGNumberFormat::kNEANonNegative,
                      TGNumberFormat::kNELLimitMinMax, 0, prt_maxch);

  fNumber2->Connect("ValueSet(Long_t)", "MyMainFrame", this, "InterestChanged()");
  (fNumber2->GetNumberEntry())
    ->Connect("ReturnPressed()", "MyMainFrame", this, "InterestChanged()");
  ...
  if (prt_geometry == 2023) {
    if (ginFile.Contains("pilas")) fEdit1->SetText("400 0  100");
    fEdit2->SetText("200 0  80");
  }
...
void cdisplay(TString inFile = "pilasM.root", Int_t trigger = 0, Int_t mode = 0, TString path = "",
              TString info = "0", Int_t entries = 0, Int_t workers = 4, int setupid = 2019) {
  ginFile = inFile;
  gEntries = entries;
  gTrigger = trigger;
  gMode = mode;
