...
void MSelector::SlaveBegin(TTree *){
  ...
  for(Int_t m=0; m<nmcp; m++){
    ...
    for(Int_t p=0; p<npix; p++){     
      ...
      if(gMode==1){
	hShape[m][p] = new TH2F(Form("hShape_mcp%dpix%d",m,p), Form("hShape_%d_%d",m,p) , bins1,min1,max1,100,-15.1,15);
	hLeTot[m][p] = new TH2F(Form("hLeTot_mcp%dpix%d" ,m,p), Form("mcp %d, pixel %d",m, p), 400,min1,max1, bins2,min2,max2);
